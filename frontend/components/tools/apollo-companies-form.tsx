...
export function ApolloCompaniesForm({ clientTag }: Props) {
  ...
  const handleExportCsv = async (items: Record<string, unknown>[]) => {
    try {
      ...
      a.href = url;
      a.download = res.filename || "apollo-companies.csv";
      a.click();
  ...
  const handleSearch = async () => {
    setLoading(true);
    try {
      const data = await api.toolsSearchCompanies({
        organization_locations: locations ? locations.split(",").map((s) => s.trim()).filter(Boolean) : undefined,
        ...
        technologies: technologies ? technologies.split(",").map((s) => s.trim()).filter(Boolean) : undefined,
        keywords: keywords || undefined,
        per_page: perPage,
        client_tag: clientTag,
      });
      setResults(data);
      toast({ title: `${data.total} aziende trovate`, description: `Mostrate ${data.results.length}` });
    } catch (err: any) {
    ...
    <div className="space-y-4">
      <div className="grid grid-cols-2 gap-3">
        ...
        <div>
          ...
          <Input placeholder="react, salesforce..." value={technologies} onChange={(e) => setTechnologies(e.target.value)} className="mt-1" />
        </div>
      ...
      <div className="flex items-center gap-3">
        <div className="flex items-center gap-2">
          ...
          <Input type="number" min={1} max={100} value={perPage} onChange={(e) => setPerPage(Number(e.target.value))} className="w-16 h-8" />
        </div>
